...
interface ProcessedBlock {
  ...
  specifications?: Array<{ id: string; name: string; value: string; order: number }>
  text?: string
  buttonText?: string
  buttonLink?: string
...
class ArticleProcessor {
  ...
  groupBlocksByType() {
    return {
      ...
      ingredients: this.allBlocks.filter((b) => b.type === "ingredients"),
      cta: this.allBlocks.filter((b) => b.type === "cta"),
      bulletLists: this.allBlocks.filter((b) => b.type === "bullet-list"),
      faqs: this.allBlocks.filter((b) => b.type === "faq"),
      specifications: this.allBlocks.filter((b) => b.type === "specifications"),
    }
  }
  ...
  extractContentSections() {
    ...
    const findContentByTitle = (titlePattern: string) => {
      const section = this.sections.find((s) => s.title.toLowerCase().includes(titlePattern.toLowerCase()))
      if (section) {
        const paragraphBlock = section.blocks.find((b: any) => b.type === "paragraph")
    ...
    return {
      overview: findSectionContent("overview") || findContentByTitle("overview"),
      description: findSectionContent("description") || findContentByTitle("what is"),
      howToTake: findSectionContent("howToTake") || findContentByTitle("how to use"),
      safety: findSectionContent("safety") || findContentByTitle("safety"),
      effectiveness: findSectionContent("effectiveness") || findContentByTitle("effective"),
      howItWorks: findSectionContent("howItWorks") || findContentByTitle("how does it work"),
      conclusion: findSectionContent("conclusion") || findContentByTitle("conclusion"),
    }
  }
      ...
      switch (block.type) {
        ...
        case "paragraph":
          return {
            ...baseBlock,
            content: block.content || "",
            section: block.customFields?.find((cf: any) => cf.name === "section")?.value || section.title,
          }
        case "image":
          return {
            ...baseBlock,
            imageUrl: block.imageUrl || "",
            caption: block.imageCaption || block.content || "",
          }
        case "list":
          return {
            ...baseBlock,
  ...
  processStructuredContent(): ProcessedBlock[] {
    // Flatten all blocks from all sections for backward compatibility
    return this.sections.flatMap((section) => this.processBlocksInSection(section.blocks, section))
  }
}
